...
export default function Home() {
  ...
  const approve = () => {
    if (!amount) return;
    writeContract({
      address: asset, abi: ERC20_ABI, functionName: 'approve',
      ...
      <main className="mx-auto max-w-6xl px-5 py-10 space-y-10">
        ...
        {isConnected && (
          <section className="grid gap-4 md:grid-cols-3">
            <div className="card p-5">
              ...
              <div className="text-xs opacity-60 mt-1">Available to deposit</div>
            </div>
            <div className="card p-5">
              <div className="text-xs opacity-60 mb-1">Your iUSDC Balance</div>
              <div className="text-2xl font-bold">{formattedIusdcBalance}</div>
              <div className="text-xs opacity-60 mt-1">Earning halal yields</div>
            </div>
            <div className="card p-5">
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <div className="col-span-2 card p-7">
            <h1 className="text-3xl font-semibold tracking-tight">Halal Savings with iUSDC</h1>
            <p className="mt-2 text-sm opacity-70 max-w-[52ch]">
              Deposit USDC to receive iUSDC. Your savings grow through shariah-compliant profit-sharing, not interest.
            </p>
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <input
                ...
                type="number"
              />
              <button onClick={approve} className="btn btn-outline">Approve</button>
              <button onClick={deposit} className="btn btn-primary" disabled={pending}>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <input
                ...
                type="number"
              />
              <button onClick={withdraw} className="btn btn-outline" disabled={pending}>
                Withdraw
              </button>
            </div>
            ...
            {isConnected && (
              <div className="mt-4 text-xs opacity-60">
                Available: {formattedUsdcBalance} USDC • {formattedIusdcBalance} iUSDC
              </div>
            )}
            
            <div className="mt-6 p-4 bg-mist rounded-lg border border-line">
              <p className="text-xs font-semibold text-blueribbon mb-1">100% Shariah-Compliant</p>
              <p className="text-sm opacity-70">
                No riba (interest). Profits come from halal business activities and are shared transparently with savers.
              </p>
            </div>
          </div>
          ...
          <div className="card p-7">
            ...
            <div className="mt-4 p-3 bg-blueribbon/5 rounded-lg">
              ...
              <p className="text-xs opacity-70 mt-1">Profit-sharing from permissible investments</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <VaultCard
              ...
              onDepositClick={() => document.querySelector<HTMLInputElement>('input[placeholder="Amount (USDC)"]')?.focus()}
              onWithdrawClick={() => document.querySelector<HTMLInputElement>('input[placeholder="Shares (iUSDC)"]')?.focus()}
        ...
        <section className="grid gap-5 md:grid-cols-3">
          <div className="card p-6">
            ...
            <p className="text-sm opacity-70">No interest, only profit-sharing from halal sources</p>
          </div>
          ...
          <div className="card p-6">
            ...
            <p className="text-sm opacity-70">Save for Hajj, Umrah, education, and more</p>
          </div>
          ...
          <div className="card p-6">
            ...
            <p className="text-sm opacity-70">All funds and profits tracked on-chain</p>
          </div>
        </section>
      </main>
